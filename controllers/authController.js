...
const createResSend = (user, statusCode, res) => {
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    //verify if the POSTed password is correct
    if (
        !(await user.correctPassword(req.body.passwordCurrent, user.password))
    ) {
        return next(new AppError("Your current password is incorrect", 401));
    }
    //if so, update current password
    user.password = req.body.password;
    user.passwordConfirm = req.body.passwordConfirm;
    await user.save(); // use save and not findByIdAndUpdate since this is the best practice for passwords
